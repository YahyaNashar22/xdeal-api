...
export const createVehicleCategory = async (req, res) => {
    try {
        ...
        const exists = await VehicleCategories.findOne({
            title: { $regex: `^${escapeRegex(title)}$`, $options: "i" },
        });
        if (exists) return res.status(409).json({ message: "Category already exists" });

        const doc = await VehicleCategories.create({ title });
...
export const getVehicleCategories = async (req, res) => {
    try {
        ...
        const [items, total] = await Promise.all([
            VehicleCategories.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit),
            VehicleCategories.countDocuments(filter),
        ]);
...
export const getVehicleCategoryById = async (req, res) => {
    try {
        ...
        const doc = await VehicleCategories.findById(id);
...
export const updateVehicleCategory = async (req, res) => {
    try {
        ...
        const updated = await VehicleCategories.findByIdAndUpdate(
            id,
            { title },
...
export const deleteVehicleCategory = async (req, res) => {
    try {
        ...
        const deleted = await VehicleCategories.findByIdAndDelete(id);
